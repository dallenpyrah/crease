...
export const clear = (model: Model, ids?: ReadonlyArray<string>): Update => {
  const annotations =
    ids === undefined
      ? []
      : model.annotations.filter((annotation) => !ids.includes(annotation.id));
  return annotations.length === model.annotations.length
    ? unchanged(model)
    : mutate(model, annotations);
};
